# Usage python astLab.py --input input2 --output output2 --threshType ast --sizeThresh True --lineDet True --eval groundTruth2 --writeDir True
...
for file in inFolder:
    ...
    if args["eval"] is not None:
        tFile = file.split(".")
        truth = cv2.imread(args["eval"] + '/' + tFile[0] + '.jpg')
        print("Evaluating with mask...")
        print(args["eval"] + '/' + tFile[0] + '.jpg')
        print("------------------------")
        filename = args["eval"] + '/' + tFile[0] + '.jpg'
